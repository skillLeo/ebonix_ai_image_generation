...
function o() {
    const lazyItems = document.querySelectorAll(
      "[data-king-img-src], .king-pvideo"
    );
    if (!lazyItems.length) return;
    const loadLazyItem = (item) => {
      if (item.hasAttribute("data-king-img-src")) {
        const src = item.getAttribute("data-king-img-src");
        if (!src) return;
        item.classList.contains("king-lazy")
          ? ((item.onload = () => {
              (item.style.height = ""),
                (item.style.width = ""),
                item.classList.add("loaded"),
                item.removeAttribute("data-king-img-src");
            }),
            (item.src = src),
            item.parentNode?.classList.add("img-loaded"))
          : ((item.style.backgroundImage = `url('${src}')`),
            item.classList.add("loaded"),
            item.removeAttribute("data-king-img-src"));
      }
      if (item.classList.contains("king-pvideo")) {
        const video = item.querySelector(".king-avideo"),
          source = video?.querySelector("source");
        if (source && !source.src) {
          const dataSrc = source.getAttribute("data-src");
          dataSrc && ((source.src = dataSrc), video.load());
        }
      }
    };
        ...
        (entries, obs) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              loadLazyItem(item), obs.unobserve(item);
            }
          });
        },
        { rootMargin: "300px" }
    ...
    } else lazyItems.forEach(loadLazyItem);
  }
