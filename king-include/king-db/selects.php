...
function aisize_ratio($imsize)
{
	if ($imsize === '1344x768') {
		$aspect_ratio = '16:9';
	} elseif ($imsize === '768x1344') {
		$aspect_ratio = '9:16';
	} elseif ($imsize === '1792x1024') {
		$aspect_ratio = '5:4';
	} elseif ($imsize === '832x1216') {
		$aspect_ratio = '4:5';
	} elseif ($imsize === '1024x1536') {
		$aspect_ratio = '2:3';
	} else {
		$aspect_ratio = '1:1';
	}
...
function king_aiposts($userid, $type)
/*
Return the selectspec to retrieve recent questions by the user identified by $identifier, where $identifier is a
handle if we're using internal user management, or a userid if we're using external users. Also include the
corresponding vote on those questions made by $userid (if not null). Return $count (if null, a default is used)
questions.
 */
{
	...
	$selectspec['source'] .= " WHERE ^posts.userid=$ AND postformat=$ AND type='NOTE' ORDER BY ^posts.created DESC";
	array_push($selectspec['arguments'], $userid, $type);
	$selectspec['sortdesc'] = 'created';

	return $selectspec;
}
	function king_update_ai_post($postid, $title, $tagstring, $nsfw = null, $format = 'I')
/*
	Update an AI-generated post with new title, tags, nsfw, and prvt fields. $format defaults to 'I'.
*/
	{
		qa_db_query_sub(
			'UPDATE ^posts SET type=$, title=$, tags=$, nsfw=$, postformat=$ WHERE postid=#',
			'Q', $title, $tagstring, $nsfw, $format, $postid
		);
	}
