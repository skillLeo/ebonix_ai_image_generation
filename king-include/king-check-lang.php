...
/*
	...
	File: king-include/king-check-lang.php
...
<html>
	<head>
		...
		<title>King-Media Language Check</title>
		<style>
			code {font-size:125%;}
		</style>
	</head>
...
<?php
	...
	echo '<h1>Checking US English files in <code>king-include</code>...</h1>';

	$includefiles=array_merge(glob(QA_INCLUDE_DIR.'king-*.php'), glob(QA_INCLUDE_DIR.'*/king-*.php'), glob(QA_PLUGIN_DIR.'*/king-*.php'));
	...
	foreach ($includefiles as $includefile) {
		...
		if (preg_match('|/king-include/king-lang-([a-z]+)\.php$|', $includefile, $matches)) { // it's a lang file
			$prefix=$matches[1];
	...
	foreach ($languages as $code => $language) {
		echo '<h1>Checking '.$language.' files in <code>king-lang/'.$code.'</code>...</h1>';

		$langdefined=array();
		$langdifferent=array();
		$langsubstitutions=array();
		$langincludefiles=glob(QA_LANG_DIR.$code.'/king-*.php');
		$langnewphrases=array();

		output_start_includes();

		foreach ($langincludefiles as $langincludefile)
			if (preg_match('/king-lang-([a-z]+)\.php$/', $langincludefile, $matches)) { // it's a lang file
				$prefix=$matches[1];
		...
		foreach ($langnewphrases as $prefix => $phrases) {
			echo '<h2>'.$language.' phrases to add to <code>king-lang/'.$code.'/king-lang-'.$prefix.'.php</code>:</h2>';

			echo 'Copy and paste this into the middle of <code>king-lang/'.$code.'/king-lang-'.$prefix.'.php</code> then translate the right-hand side after the <code>=></code> symbol.';
	...
	function output_lang_issue($prefix, $key, $issue, $error=true)
	{
		...
		echo 'king-lang-<b>'.qa_html($prefix).'</b>.php:';
