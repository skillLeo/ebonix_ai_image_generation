...
/*
	...
	File: king-include/king-external-users-wp.php
...
*/
	...
	function qa_get_logged_in_user_html($logged_in_user, $relative_url_prefix)
	{
		...
		return '<a href="'.qa_path_html('user/'.$publicusername).'" class="king-user-link">'.htmlspecialchars($publicusername).'</a>';
	}
	...
	function qa_get_users_html($userids, $should_include_link, $relative_url_prefix)
	{
		...
		foreach ($userids as $userid) {
			...
			if ($should_include_link)
				$usershtml[$userid]='<a href="'.qa_path_html('user/'.$publicusername).'" class="king-user-link">'.$usershtml[$userid].'</a>';
		}
