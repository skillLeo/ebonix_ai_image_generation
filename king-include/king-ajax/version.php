...
/*
	...
	File: king-include/king-ajax-version.php
...
*/
	...
	if (strlen(@$metadata['version'])) {
		if (strcmp($metadata['version'], $version)) {
			if (qa_qa_version_below(@$metadata['min_KINGMEDIA'])) {
				$response = strtr(qa_lang_html('admin/version_requires_KINGMEDIA'), array(
					'^1' => qa_html('v'.$metadata['version']),
					'^2' => qa_html($metadata['min_KINGMEDIA']),
				));
			}
