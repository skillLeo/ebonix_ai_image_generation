...
/*
	...
	File: king-include/king-index.php
	Description: The Grand Central of KINGMEDIA - most requests come through here
...
//	If this is an special non-page request, branch off here
	...
	if (isset($_POST['qa']) && $_POST['qa'] == 'ajax')
		require 'king-ajax.php';

	elseif (isset($_GET['qa']) && $_GET['qa'] == 'image')
		require 'king-image.php';

	elseif (isset($_GET['qa']) && $_GET['qa'] == 'blob')
		require 'king-blob.php';

	else {

	//	Otherwise, load the KINGMEDIA base file which sets up a bunch of crucial stuff
		 ...
		 *   Apache king-rewrite unescapes characters, converts `+` to ` `, cuts off at `#` or `&`
		 *   Nginx king-rewrite unescapes characters, retains `+`, contains true path
		 */
		function qa_index_set_request()
		{
			...
			if (isset($_GET['king-rewrite'])) { // URLs rewritten by .htaccess or Nginx
				$urlformat = QA_URL_FORMAT_NEAT;
				$qa_rewrite = strtr(qa_gpc_to_string($_GET['king-rewrite']), '+', ' '); // strtr required by Nginx
				$requestparts = explode('/', $qa_rewrite);
				unset($_GET['king-rewrite']);
				...
				if (!empty($_SERVER['REQUEST_URI'])) { // workaround for the fact that Apache unescapes characters while rewriting
					...
					// Generally we assume that $_GET['king-rewrite'] has the right path depth, but this won't be the case if there's
					// a & or # somewhere in the middle of the path, due to Apache unescaping. So we make a special case for that.
					// If 'REQUEST_URI' and 'king-rewrite' already match (as on Nginx), we can skip this.
	...
	//	Branch off to appropriate file for further handling
		...
		elseif ($requestlower == 'url/test/'.QA_URL_TEST_STRING)
			require QA_INCLUDE_DIR.'king-url-test.php';
		else {
			...
			if (substr($requestlower, 0, 5) == 'feed/')
				require QA_INCLUDE_DIR.'king-feed.php';
			else
				require QA_INCLUDE_DIR.'king-page.php';
		}
	}
